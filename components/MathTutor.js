        ...
        if (!response.ok) {
          ...
          const friendly =
            response.status === 503 || /not configured|api key/i.test(serverMsg)
              ? "I'm almost ready! 🛠️ The tutor's AI connection hasn't been set up yet — whoever set up this site needs to add an ANTHROPIC_API_KEY, OPENAI_API_KEY, or GEMINI_API_KEY. (See the README.)"
